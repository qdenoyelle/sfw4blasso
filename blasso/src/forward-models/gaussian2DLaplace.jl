type gaussian2DLaplace <: discrete
  dim::Int64
  rl_model::Bool
  px::Array{Float64,1}
  py::Array{Float64,1}
...
function setKernel(rl_model::Bool,Npx::Int64,Npy::Int64,pz::Array{Float64,1},Dpx::Float64,Dpy::Float64,
  ...
  nbpointsgrid=[convert(Int64,round(5*lSample[i]*abs(b[i]-a[i]),0)) for i in 1:dim];
...
type gaussconv2Ddnlaplace <: operator
    ker::DataType
    dim::Int64
    rl_model::Bool
    sigmax::Float64
...
function setoperator(kernel::gaussian2DLaplace,a0::Array{Float64,1},x0::Array{Array{Float64,1},1},w::Array{Float64,1})
  ...
  function correl(x::Array{Float64,1},Phiu::Array{Array{Array{Float64,1},1},1})
    ...
    c=[vecdot(phiz(x[3]),Phiu[3][i]) for i in 1:length(Phiu[3])];
    return vecdot(a,b.*c)-ob(x);
  end
  function d1correl(x::Array{Float64,1},Phiu::Array{Array{Array{Float64,1},1},1})
    ...
    dc=[vecdot(d1phiz(x[3]),Phiu[3][i]) for i in 1:length(Phiu[3])];
    d1c[1]=vecdot(da,b.*c)-d1ob(1,x);
    d1c[2]=vecdot(a,db.*c)-d1ob(2,x);
    d1c[3]=vecdot(a,b.*dc)-d1ob(3,x);
    return d1c
  end
  function d2correl(x::Array{Float64,1},Phiu::Array{Array{Array{Float64,1},1},1})
    ...
    ddc=[vecdot(d2phiz(x[3]),Phiu[3][i]) for i in 1:length(Phiu[3])];
    d2c[1,2]=vecdot(da,db.*c)-vecdot(d11phiVect(1,2,x),y);
    d2c[1,3]=vecdot(da,b.*dc)-vecdot(d11phiVect(1,3,x),y);
    d2c[2,3]=vecdot(a,db.*dc)-vecdot(d11phiVect(2,3,x),y);
    d2c=d2c+d2c';
    d2c[1,1]=vecdot(dda,b.*c)-d2ob(1,x);
    d2c[2,2]=vecdot(a,ddb.*c)-d2ob(2,x);
    d2c[3,3]=vecdot(a,b.*ddc)-d2ob(3,x);
    return(d2c)
  end
...
function minCorrelOnGrid(Phiu::Array{Array{Array{Float64,1},1},1},kernel::blasso.gaussian2DLaplace,op::blasso.operator,positivity::Bool=true)
  ...
  l=1;
  for k in 1:length(kernel.grid[3])
    c=[vecdot(op.Phiz[k],Phiu[3][m]) for m in 1:length(Phiu[3])];
    for i in 1:length(kernel.grid[1])
      a=[vecdot(op.Phix[i],Phiu[1][m]) for m in 1:length(Phiu[1])];
      for j in 1:length(kernel.grid[2])
        b=[vecdot(op.Phiy[j],Phiu[2][m]) for m in 1:length(Phiu[2])];
        buffer=vecdot(a,b.*c)-op.PhisY[l];
        if !positivity
          buffer=-abs(buffer);
        end
